...
capacity:Test(
  ...
  function(ctx)
    ...
    ctx:Expect(tostring(reported[1]):find("has 2 objects borrowed at once", 1, true)).Not:ToBeNil()
...
allocation:Test(
  ...
  function(ctx)
    ...
    ctx:Log(
      ("memory delta over %d served requests: %.3f KB"):format(ALLOCATION_CYCLES, grownKilobytes)
    )
    ctx:Expect(grownKilobytes <= ALLOCATION_TOLERANCE_KB):ToBe(true)
