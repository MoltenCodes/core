...
dispatch:Test(
  ...
  function(ctx)
    ...
    ctx:Log(
      ("payload: %s, %s"):format(describeFact(received.total), describeFact(received.thisLevel))
    )
...
isolation:Test(
  ...
  function(ctx)
    ...
    ctx:Log(
      ("the collector received %d report(s), %d of them this test's"):format(#reported, #ownReports)
    )
...
combatLog:Test(
  ...
  function(ctx)
    local globalReader =
      logFunctionFact(ctx, "CombatLogGetCurrentEventInfo", readHost("CombatLogGetCurrentEventInfo"))
    ...
    if REGISTRATIONS_READABLE then
      ...
      if listed then
        ...
        ctx:Log(
          "GetFramesRegisteredForEvent(COMBAT_LOG_EVENT_UNFILTERED) raised: " .. tostring(frames)
        )
      end
    end
