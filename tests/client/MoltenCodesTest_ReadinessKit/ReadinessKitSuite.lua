...
negativeCache:Test(
  ...
  function(ctx)
    ...
    if spunUntil < expiry then
      ctx:Fail(("GetTimePreciseSec did not advance past the interval in %d reads"):format(spins))
    end
    gate:Probe()
    ctx:Log(
...
eventTest(
  ...
  function(ctx)
    ...
    ctx:Log("the client has C_EventUtils.IsEventValid: " .. tostring(type(isEventValid) ~= "nil"))
    ...
    for _ = 1, 2 do
      ...
      ctx
        :Expect(text:sub((afterPosition or 0) + 1, (afterPosition or 0) + #expected))
        :ToBe(expected)
    end
    ctx:Expect(gate:ReprobeOn(CVAR_EVENT)):ToBe(true)
  end
...
allocation:Test(
  ...
  function(ctx)
    ...
    ctx:Log(
      ("memory delta over %d rounds of 5 calls: %.3f KB"):format(ALLOCATION_ROUNDS, grownKilobytes)
    )
...
secretTest(
  ...
  function(ctx)
    ...
    for _, plain in ipairs({ true, false }) do
      ...
      local succeeded, problem = pcall(ReadinessKit.Gate, ReadinessKit, name, function()
        return secretAnswer
      end, { intervalSeconds = POLL_INTERVAL_SECONDS, timeoutSeconds = SHORT_TIMEOUT_SECONDS })
      ...
      local own = 0
      for _, message in ipairs(reported) do
        if type(message) == "string" and not isSecret(message) and message == expectedReport then
          own = own + 1
        end
      end
