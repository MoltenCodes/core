...
formatSuite:Test(
    ...
    function(ctx)
        ...
        ctx:Expect(LocaleKit:Format("%2$d items belong to %1$s", "Alice", 3))
...
formatSuite:Test(
    ...
    function(ctx)
        ...
        for _, case in ipairs(cases) do
            ...
            if accepted then
                compared = compared + 1
                ctx:Expect(LocaleKit:Format(case.template, unpack(case.values, 1, count)))
                    :ToBe(native)
            end
        end
        if compared == 0 then
...
errors:Test(
    ...
    function(ctx)
        ...
        expectErrorAtNextLine(
            ...
            "LocaleKit:GetLocale "
                .. probeName
                .. ' already uses missing mode "silent", not "report"'
        )
