...
secureMethodTest(
    ...
    function(ctx)
        ...
        local scope = newScope()
        local received = nil
        ctx:Expect(scope:SecureHook(probe, "Combine", function(...)
            received = pack(...)
        end)):ToBe(true)
...
scripts:Test(
    ...
    function(ctx)
        ...
        ctx:Expect(scope:SecureHookScript(frame, "OnShow", function(received)
            seen[#seen + 1] = "show"
            shownFrame = received
        end)):ToBe(true)
        ctx:Expect(scope:SecureHookScript(frame, "OnHide", function()
            seen[#seen + 1] = "hide"
        end)):ToBe(true)
...
scripts:Test(
    ...
    function(ctx)
        ...
        local scope = newScope()
        ctx:Expect(scope:HookScript(frame, "OnShow", function(received)
            order[#order + 1] = "pre"
...
scripts:Test(
    ...
    function(ctx)
        ...
        ctx:Expect(scope:RawHookScript(frame, "OnHide", function(previous, received)
            calls = calls + 1
...
access:Test(
    ...
    function(ctx)
        ...
        ctx:Log(
            ("getmetatable(frame) is a %s; its __index is a %s"):format(
                type(metatable),
                type(index)
            )
        )
...
access:Test(
    ...
    function(ctx)
        ...
        if type(inCombatLockdown) ~= "function" or not inCombatLockdown() then
            Harness:SkipTest(
                ctx,
                "the player is not in combat; this passive test never starts combat"
            )
            return
        end
        local button = protectedButton()
...
release:Test(
    ...
    function(ctx)
        ...
        ctx:Expect(pack(scope:Hook(probe, "Format", ignore)))
