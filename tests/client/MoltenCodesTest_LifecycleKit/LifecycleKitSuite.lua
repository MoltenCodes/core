...
phases:Test(
  ...
  function(ctx)
    ...
    ctx:Log("C_AddOns.IsAddOnLoaded answered finished " .. describeFact(fileFacts.addonFinished))
...
combatDeferral:Test(
  ...
  function(ctx)
    ...
    local run = runs[1]
    ctx:Log("inside the deferred call, InCombatLockdown() answered " .. describeFact(run.lockdown))
    ...
    ctx:Expect(call:IsPending()):ToBe(false)
    ctx:Expect(order):ToEqual({ "deferred call", "OnCombatEnd", "PLAYER_REGEN_ENABLED listener" })
  end
)
