...
MoltenCodes Test: PASS apiKit.calls: read-only getters called through the wrapper answer exactly what the raw calls answer: GetBuildInfo, the build probes, GetLocale, UnitName, UnitClass, C_AddOns, C_CVar.GetCVar, C_EventUtils.IsEventValid, InCombatLockdown and others
...
| `named samples are the host's own functions ...` | Eleven readable pairs, such as `api.timer.newTicker == C_Timer.NewTicker` and `api.unit.name == UnitName`, and `api.profiler` is the same table as `api.addOnProfiler`. |
...
| `read-only getters called through the wrapper ...` | Twenty-three getters (`GetBuildInfo`, the build probes, `GetLocale`, `GetCurrentRegion`, `GetExpansionLevel`, `IsLoggedIn`, `UnitName`, `UnitClass`, `UnitLevel`, `UnitFactionGroup`, `UnitGUID`, `GetRealmName`, `C_AddOns`, `C_CVar.GetCVar`, `C_EventUtils.IsEventValid`, `InCombatLockdown`, `C_Map.GetBestMapForUnit`) return the same number of values and the same values through the wrapper as raw. |
