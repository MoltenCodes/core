...
local function waitForControlTimer(ctx)
    ...
    if
        not waitUntil(ctx, function()
            return controlFired
        end, CONTROL_TIMEOUT_SECONDS)
    then
        ctx:Fail(
...
lifecycle:Test(
    ...
    function(ctx)
        ...
        ctx:Log(
            "first request for the container in this session: "
                .. tostring(not containerAskedBefore)
        )
...
lifecycle:Test(
    ...
    function(ctx)
        ...
        local module =
            trackModule(ownContainer():CreateModule(newNamePrefix() .. "Late", definition))
...
ordering:Test(
    ...
    function(ctx)
        ...
        container:DisableAll()
        ctx:Expect(journalLabels(journal, "disable"))
            :ToEqual({ "Interface", "Data", "Core", "Early" })
...
scopes:Test(
    ...
    function(ctx)
        ...
        local module =
            trackModule(ownContainer():CreateModule(newNamePrefix() .. "Timers", definition))
...
if REGISTRATIONS_READABLE then
    scopes:Test(eventsTestName, function(ctx)
        ...
        local module =
            trackModule(ownContainer():CreateModule(newNamePrefix() .. "Events", definition))
        ...
        ctx:Log(
            ("deliveries: %d while enabled, %d in all"):format(deliveredWhileEnabled, deliveries)
        )
...
scopes:Test(
    ...
    function(ctx)
        ...
        local module =
            trackModule(ownContainer():CreateModule(newNamePrefix() .. "Hooks", definition))
...
scopes:Test(
    ...
    function(ctx)
        ...
        local module =
            trackModule(ownContainer():CreateModule(newNamePrefix() .. "Others", definition))
    ...
    function(ctx)
        ...
        expectErrorAtCallingLine(ctx, function(at)
            ...
            container:CreateModule(
                name,
                { implements = { "OnEnable", "OnDisable" }, onEnable = ignore }
            )
