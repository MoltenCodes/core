...
disabled:Test(
    ...
    function(ctx)
        ...
        ctx:Log(
            ("memory delta over %d disabled Measure calls: %.3f KB"):format(CALLS, grownKilobytes)
        )
...
allocation:Test(
    ...
    function(ctx)
        ...
        ctx:Log(
            ("memory delta over %d enabled Begin/End pairs: %.3f KB"):format(CALLS, grownKilobytes)
        )
        ctx:Expect(grownKilobytes <= ALLOCATION_TOLERANCE_KB):ToBe(true)
...
allocation:Test(
    ...
    function(ctx)
        ...
        ctx:Log(
            ("memory delta over %d enabled Measure calls: %.3f KB"):format(CALLS, grownKilobytes)
        )
        ctx:Expect(grownKilobytes <= ALLOCATION_TOLERANCE_KB):ToBe(true)
        ctx:Expect(expectRow(ctx, SECTION.allocation).count):ToBe(CALLS + 1)
    end
...
limits:Test(
    ...
    function(ctx)
        ctx:Expect(select("#", ProfileKit:SetLimits({ maxSections = ProfileKit.UNBOUNDED })))
            :ToBe(0)
