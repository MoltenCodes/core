...
clockTest(
    ...
    function(ctx)
        ...
        for index = 2, math.min(#starts, #finishes + 1) do
            ...
            ctx:Expect(gap >= TICK_INTERVAL_SECONDS - EARLY_WAKE_SECONDS - TIMING_MARGIN_SECONDS)
                :ToBe(true)
...
clockTest(
    ...
    function(ctx)
        ...
        ctx:Expect(report:find("exceeded the cooperative slice threshold", 1, true) ~= nil)
            :ToBe(true)
        ctx:Expect(report:find("> " .. DEFAULT_RUNAWAY_THRESHOLD_MS .. "ms", 1, true) ~= nil)
            :ToBe(true)
    end
)
