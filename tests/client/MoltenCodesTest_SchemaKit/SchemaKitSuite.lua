...
allocation:Test(
    ...
    function(ctx)
        ...
        expectNoAllocation(ctx, "5000 string checks", function()
            if
                Label:Check("Auto Attack")
                and Anchor:Check("CENTER")
                and Outline:Check("OUTLINE")
            then
                accepted = accepted + 1
            end
        end)
...
errors:Test(
    ...
    function(ctx)
        ...
        for _, method in ipairs({ "Check", "Assert" }) do
            ...
            ctx:Expect(tostring(message):match(": (.*)$"))
                :ToBe("the custom check raised on purpose")
