...
clockTest(
    ...
    function(ctx)
        ...
        for index = 1, ticksAtCancel do
            ...
            ctx:Expect(deadlineLeads[index] <= TICK_INTERVAL_SECONDS + CLOCK_EPSILON_SECONDS)
                :ToBe(true)
            ctx
                :Expect(
                    deadlineLeads[index] >= TICK_INTERVAL_SECONDS - TICK_DEADLINE_EPSILON_SECONDS
                )
                :ToBe(true)
        end
...
clockTest(
    ...
    function(ctx)
        ...
        ---@cast deadline number
        ctx:Log(
            ("deadline %s after the reading before Start"):format(milliseconds(deadline - before))
        )
...
clockTest(
    ...
    function(ctx)
        ...
        waitUntilInstant(ctx, preciseNow() + QUIET_WAIT_SECONDS)
        ctx:Log(
            ("probe %s: ticks after the close: %d"):format(probeName, ticks.count - ticksAtClose)
        )
...
clockTest(
    ...
    function(ctx)
        ...
        ctx:Log(
            ("the collector received %d report(s), %d of them this test's"):format(#reported, #own)
        )
...
allocation:Test(
    ...
    function(ctx)
        ...
        ctx:Log(
            ("memory delta over %d rounds of 11 calls: %.3f KB"):format(
                GETTER_CALLS,
                grownKilobytes
            )
        )
