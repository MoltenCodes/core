...
describe("SettingsKit profiles", function()
    ...
    it(
        "switches profile, creates it, records it per character and fires OnProfileChanged",
        function()
            local db = SettingsKit:Open("MyAddonDB", schema())
    ...
    it(
        "deletes another profile, forgets characters that used it, and detaches its views",
        function()
            local db = SettingsKit:Open("MyAddonDB", schema())
            db:SetProfile("Old")
            db.profile.scale = 2
