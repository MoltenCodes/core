...
describe("SettingsKit error levels", function()
  ...
  it("points Open argument errors at the caller", function()
    local cases = {
      ...
      },
      {
        ...
        function()
          SettingsKit:Open("MyAddonDB", { profile = S.table({ fields = { x = S.number() } }) })
        end,
      },
      {
  ...
  it("points Open saved-table errors at the caller", function()
    ...
    assertReportedAtCaller("SettingsKit:Open migration 2 of MyAddonDB failed: nope", function(mark)
      mark()
