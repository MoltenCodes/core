...
- duplicate embedded loading, Registry publication, yielding to a newer revision, missing Registry, SchemaKit or SignalKit, an incomplete facade, loading without EventKit, an in-place upgrade to the next revision that keeps the database, its views, its listeners and its logout compaction, and the upgrade over revision 1 repairing the defaults of entry views revision 1 built below a keyed section without a default;
