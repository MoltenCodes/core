...
Where a value can come from outside SettingsKit (an argument, an option, a saved-variable entry, a schema default), its absence is tested with `type(value) == "nil"`, never by comparing it with `nil`: that is the repository rule, which never compares anything. On a client with secret values a secret compared with a value of its own type raises, and so does a secret used as a key; a comparison with `nil` happens not to raise (measured on Retail 12.1.0 b69933). A saved value, a secret included, is read back as it is stored. The secret refusals above, with their exact messages, are the only new errors this adds:
