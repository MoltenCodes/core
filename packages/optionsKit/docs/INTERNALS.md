...
The tree prototype is published as `OptionsKit.Tree`. OptionsKit hands out no closures that outlive a call except the dynamic key checks inside schemas (below) and the callbacks and signal listener of a profile group, so it needs no dispatch table: an upgrade replaces the prototype's methods and every existing tree sees them at once. A profile group's callbacks close over its link record, the database and the file-local helpers of the revision that built them, so they keep working after an upgrade with that revision's behaviour; only groups built after a newer copy loads get its helpers.
...
| `suppress` | `true` while the `current` select's own `Set` is switching the profile, so the link's listener stays quiet and `Set` fires `OnChange` once. Restored to its previous value, not cleared, so a switch made from a listener of an outer switch leaves the outer one suppressed; restored under `pcall` so a failing switch cannot leave it set. The `new` input switches unsuppressed: the current profile changed, and only the link can announce `current`. |
| `tree`, `connections` | Set by `attachProfileLink` at the end of `Define`: the four SignalKit connections to the database's profile signals are made first, one `pcall` each so a connect that raises disconnects the ones before it, and the tree is recorded last. `false` while the group is in no tree. The listener closes over the `current` option's path, built from the group's path (`""` at the root). |
...
A newer revision inherits `state` as it is: the tree metatable, the prototype and every tree. It rewrites the prototype's methods and `runtimeRevision`. Records carry no methods and no behaviour of their own beyond the consumer's functions and the schema closures, so nothing else needs migrating. A revision that changes the record or tree layout reads `_schema` to upgrade old trees: revision 2 and later add `state.profileGroups` when it is missing and give every layout 1 tree an empty `_profileLinks` (a revision 1 tree cannot hold a profile group), then stamp it layout 2. Revisions 3 and 4 changed no layout. `validateStateBase` checks the fields every revision shares; `validateCurrentState` also requires `profileGroups`, which is what tells a finished revision 2 or later state from one still being committed.
