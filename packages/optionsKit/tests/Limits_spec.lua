...
describe("OptionsKit limits", function()
  ...
  it("honours maxOptions and accepts UNBOUNDED for it", function()
    ...
    local open = OptionsKit:Define("Open", flatTree(5000), { maxOptions = OptionsKit.UNBOUNDED })
  ...
  it("honours maxDynamicEntries on a values table", function()
    ...
    local ok, value =
      pcall(OptionsKit.Define, OptionsKit, "Lowered", selectTree(3), { maxDynamicEntries = 2 })
    assert.is_false(ok)
    assert.is_truthy(tostring(value):find("must have at most 2 entries", 1, true))

    local open =
      OptionsKit:Define("Open", selectTree(5000), { maxDynamicEntries = OptionsKit.UNBOUNDED })
