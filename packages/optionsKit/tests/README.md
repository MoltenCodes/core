...
- `ProfileOptions` against a real SettingsKit database: the group's shape (every child and its kind, the confirmations, the usage hint), the current-profile choices with the built-in `"Default"` and character profiles (and without the character one when the client does not know the player), switching through `Set` (one `OnChange`), creating through the input with SettingsKit's name rules as messages, copying, resetting and deleting with the `disabled` buttons and the refusal of an unset select at the caller, the buttons disabling when the chosen profile becomes current or is deleted elsewhere, `OnChange` on every profile signal until `Undefine` disconnects the link, the live descriptions, the `localize` hook (every documented key seen with an English default, a non-string answer ignored), the `name`/`order`/`description` overrides, the group as a tree's root, a SettingsKit listener's error propagating from `Set`, a group refused in a second tree or twice in one and freed by `Undefine`, malformed options, a `db` that is not a database, and the refusal without SettingsKit;
- duplicate embedded loading, Registry publication, yielding to a newer revision, missing Registry, SchemaKit and SignalKit, an incomplete facade, an in-place upgrade to the next revision that keeps trees and listeners, and the upgrade of a revision 1 layout (the profile group map added, each tree given its link list);
...
`support/OptionsKitTestEnv.lua` loads Registry, SignalKit, SchemaKit and OptionsKit. SettingsKit is declared under `optionalDependencies`. The stub-based specs do not depend on its package: `InstallSettingsKitStub` registers a stand-in under `settingsKit` API 1 so `Registry:Find` finds it, and `NewDatabase` builds a database with the documented shape — six scope tables whose reads fall back to defaults through a metatable, nested defaults as nested live tables, and `OnChange` and `Validate` methods. `ProfileOptions_spec.lua` requires the real SettingsKit (`require("SettingsKit")` after the chain is loaded) and installs `UnitName` and `GetRealmName` itself for the character profile choice. `SettingsKitIntegration_spec.lua` binds to a real SettingsKit database — a set and reset, a value the database refuses (`Validate` returning SettingsKit's message, `Set` raising the same text at the caller), a nested bind through a record without a default across a profile switch, a SettingsKit listener's error re-raised as it is rather than as a refusal, bound table values in `Describe`, no allocation on `Validate` of a bound option, and a bind to an undeclared scope refused at the caller — whenever `SettingsKit.lua` is on `LUA_PATH`, which the runner arranges; it is pending otherwise.
