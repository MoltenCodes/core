...
describe("OptionsKit:Define", function()
  ...
  it("accepts every option kind", function()
    local tree = OptionsKit:Define("Addon", {
      ...
      args = {
        ...
        keybinding = with({ type = "keybinding", name = "Key" }, accessors(store, "keybinding")),
        execute = {
          type = "execute",
          name = "Run",
          func = function() end,
  ...
  it("copies the tree: later edits to the tables passed have no effect", function()
    local values = { a = "A" }
    local spec =
      rootWith(with({ type = "select", name = "Select", values = values }, accessors(store, "v")))
  ...
  it("refuses value options without a reader and writer, or with both kinds", function()
    assertRefused("needs get and set functions, or bind", rootWith({ type = "toggle", name = "t" }))
    assertRefused(
  ...
  it("refuses malformed kind fields", function()
    local function range(extra)
      return rootWith(
        with(with({ type = "range", name = "r", min = 0, max = 1 }, extra), accessors(store, "r"))
      )
    end
    ...
    assertRefused("values keys must be strings or numbers", select({ values = { [true] = "yes" } }))
    ...
    local function input(extra)
      return rootWith(with(with({ type = "input", name = "i" }, extra), accessors(store, "i")))
    end
  ...
  it("refuses malformed Define options and a database without SettingsKit", function()
    assertRefused("OptionsKit:Define options must be a table", { type = "group", args = {} }, true)
    assertRefused(
  ...
  it("refuses an addon name that is not a non-empty string", function()
    ...
    TestEnv.expectErrorContaining("OptionsKit:Get addonName must be a non-empty string", function()
      OptionsKit:Get(nil)
    end)
    TestEnv.expectErrorContaining(
