...
- argument and state refusals reported at the caller's line and naming the public method (`ErrorLevels_spec.lua`): container and module arguments, provider registration and resolution, definition-table fields and entries at the `CreateModule` line, a definition's catch-up failure, `DisableAll`'s graph error, the method named after shutdown, a deterministic unknown-option report and the `SetLimits` refusals, including an unknown limit name;
...
- `implements` contracts (`Implements_spec.lua`): the list form on a value, a singleton, a module-scoped and a transient provider and on a module definition, a missing member, a member that is not a function, a value that is not a table, every malformed list and options table refused at the caller's line, the once-per-value rule (a singleton's members are looked up once, a transient's on every resolution), a refused injected value recorded as the module's failure, the schema form against the real SchemaKit (sealed schema and unsealed node, path and root failures, an inherited method that a schema does not see, a module definition against an open table schema, an impostor table) and the refusal of the schema form when SchemaKit is not loaded (the chain is loaded without SchemaKit through `NewPackageWithoutOptionalKits`; the schema stand-in is an empty table whose protected metatable name is SchemaKit's); the upgrade cases live in `Bootstrap_spec.lua`;
- secret values on the `mainline` host (`SecretValues_spec.lua`): secret names, `requiresAddons` entries, `implements` entries, the dependency policy (kept unchanged), limit values and limit names, keys of definition, options, `implements` and alias-map tables, and a secret or secret-named `requestingModule` refused at the caller's line before any comparison or key use; a secret scope key read as `nil`, an `implements` list with a secret `__metatable` read as a list, a secret provided value accepted and resolved unchanged, and ordinary arguments still accepted.
...
| `ErrorLevels_spec.lua` | argument and state refusals at the caller's line, from every public entry and from definition tables |
| `Bootstrap_spec.lua` | duplicate loading, Registry publication, dispatch repair, in-place upgrades, the manifest revision, limits and the sentinel carried across upgrades from revision 13 and to a newer revision, a revision-14 provider resolving unchecked, a contract kept under a newer revision, the upgrade from revision 15, and the upgrade from the previous revision, keeping containers, modules, providers, the dependency policy and the limits |
| `SecretValues_spec.lua` | secret arguments, policies and table keys refused at the caller's line, secret provided values accepted |
