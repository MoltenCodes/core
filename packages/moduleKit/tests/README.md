...
- secret values on the `mainline` host (`SecretValues_spec.lua`): secret names, `requiresAddons` entries, `implements` entries and limits refused at the caller's line before any comparison, a secret provided value accepted and resolved unchanged, and ordinary arguments still accepted.
...
| `SecretValues_spec.lua` | secret arguments refused at the caller's line, secret provided values accepted |
