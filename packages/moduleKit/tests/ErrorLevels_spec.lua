...
describe("ModuleKit error levels", function()
  ...
  it("points provider registration and resolution errors at the caller", function()
    ...
    expectCallerError("ModuleKit.Addon:Resolve providerName must be a non-empty string", function()
      addon:Resolve("")
    end)
    expectCallerError(
    ...
    expectCallerError("ModuleKit.Module:Resolve providerName must be a non-empty string", function()
      module:Resolve("")
    end)
  end)
  ...
  it("points the module's graph and injection errors at the caller", function()
    local module = addon:CreateModule("UI")
    expectCallerError("ModuleKit.Module:DependsOn moduleName must be a non-empty string", function()
      module:DependsOn("")
    end)
    expectCallerError("ModuleKit.Module:Inject map aliases must be non-empty strings", function()
  ...
  it("points definition-table errors at the CreateModule line", function()
    ...
    expectCallerError('ModuleKit module definition contains unknown field "dependOn"', function()
  ...
  it("points a definition's catch-up failure at the CreateModule line", function()
    ...
    expectCallerError('ModuleKit module "Late" requires missing dependency "Missing"', function()
  ...
  it("points SetLimits refusals at the caller", function()
    ...
    expectCallerError("ModuleKit:SetLimits limits.maxModules is not a recognised limit", function()
  ...
  it("names the first unknown option in sorted order", function()
    expectCallerError(
      ...
      function()
        addon:ProvideValue("Config", {}, { zeta = true, alpha = true, implements = { "Save" } })
      end
    )
  end)
end)
