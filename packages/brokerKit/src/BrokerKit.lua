...
local function validateDefinition(definition, level)
  ...
  if maxAttributes ~= UNBOUNDED and count > maxAttributes then
    error("BrokerKit:New refuses more than " .. maxAttributes .. " attributes on an object", level)
  end
  return count
end
...
local function packageNew(self, name, definition)
  ...
  if existing ~= nil then
    if existing.foreign then
      error(
        'BrokerKit:New name "' .. name .. '" is already taken by a foreign LibDataBroker object',
        2
      )
    end
  ...
  if not underObjectLimit() then
    error("BrokerKit:New refuses more than " .. rawget(sharedLimits, "maxObjects") .. " objects", 2)
  end
