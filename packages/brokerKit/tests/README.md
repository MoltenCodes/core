...
- `OnChange`: the any list with `(object, attribute, value, previous)`, reached with the attribute omitted or `nil`, per-attribute lists, the attribute list before the any list, no fire for a same-value write or a `nil` over nothing, fires for clearing and for an equal-but-different table, the connection's lifetime, the new value visible to a listener, a listener error propagating after the value was stored, and argument refusals;
