...
- limits: the default of 1024 enforced, `options.maxMissingKeys` smaller and larger, `LocaleKit.UNBOUNDED` recording 3000 keys, the limit fixed by the first call (a later call naming the same limit or none accepted, one naming a different limit refused at the caller with nothing changed), invalid and secret values refused at the caller with nothing changed (also when the mode is refused), and the sentinel and each addon's limit kept across an upgrade;
