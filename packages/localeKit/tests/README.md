...
- limits: the default of 1024 enforced, `options.maxMissingKeys` smaller and larger, `LocaleKit.UNBOUNDED` recording 3000 keys, the limit fixed by the first call (a later call naming the same limit or none accepted, one naming a different limit refused at the caller with nothing changed), invalid and secret values refused at the caller with nothing changed (also when the mode is refused), the secret probe asked before the value meets `UNBOUNDED`, and the sentinel and each addon's limit kept across an upgrade;
- `MissingKeys`: sorted, a new array per call, per addon, empty for an unknown addon, a bad addon name refused;
- `Format`: sequential `%s`, `%d`, `%.2f`, flags and width, indexed reordering and repetition, mixed indexed and unindexed specifiers, `%%`, an index beyond the arguments, index zero, unsupported and truncated specifiers, wrong argument types, specifiers `string.format` refuses (width over 99, repeated flags) reported by name, no argument retained after a call or a refusal, and secret arguments and a secret template refused through an `issecretvalue` stub looked up at call time; a secret key handed back by a read table without being stored or reported on a stub host that lets the read reach `__index` (Retail 12.1 refuses the key at the index; the real-client suite checks that);
...
- duplicate embedded loading, Registry publication, yielding to a newer revision, missing Registry, an incomplete facade, and an in-place upgrade that keeps tables, modes, missing keys, older proxies and the override, one that keeps `UNBOUNDED` and each addon's `maxMissingKeys`, and one from revision 1 to the working file;
