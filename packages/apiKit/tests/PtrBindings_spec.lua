...
--- The committed Public Test Realm flavour file, loaded the way an addon embeds it: after
--- the facade, on a Public Test Realm client, against the shared fixture's host. The
...
describe("ApiKit Public Test Realm bindings", function()
    after_each(function()
        package.loaded["flavours.Ptr"] = nil
        TestEnv.Reset()
    end)
    ...
    local function committedBuild()
        local file = assert(io.open("packages/apiKit/metadata/ptr/provenance.json", "r"))
    ...
    it(
        "installs on a Public Test Realm client and binds the host's functions by direct alias",
        function()
            local ApiKit = TestEnv.NewPackageFor("ptr")
            require("flavours.Ptr")

            -- selene: allow(global_usage)
            local api = rawget(_G, "MoltenCodes").wow.ptr.api
            -- selene: allow(global_usage)
            ...
            local version, build = ApiKit:GetMetadataBuild("ptr")
    ...
    it("costs nothing but its registration on another flavour", function()
        ...
        assert.are.same({}, root.ptr.api)
        assert.are.same({}, root.retail.api)
        local _, build = ApiKit:GetMetadataBuild("ptr")
        assert.are.equal(committedBuild(), build)
    end)
    ...
    it("refuses to load before the facade", function()
        ...
        TestEnv.SetClient({ projectId = 1, testBuild = true, betaBuild = false })
        require("Registry")
        TestEnv.expectErrorContaining("requires ApiKit API 1 to be loaded first", function()
            TestEnv.requireAfterFailedLoad("flavours.Ptr")
        end)
    end)
end)
