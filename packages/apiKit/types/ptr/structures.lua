...
---@field showCooldownFrame boolean
---@field showCooldownEdge boolean
---@field showCountdownNumbers boolean
---@field showDispelIcon boolean
...
---Optional background texture asset.
---@field bgFile? TextureAssetDisk
---Optional edge texture asset.
---@field edgeFile? TextureAssetDisk
---Optional edge size.
---@field edgeSize? number
---Optional backdrop insets.
---@field insets? AuraContainerTooltipBackdropInsets
---If true, tiles the background texture.
---@field tile? boolean
---If true, tiles the edge texture.
---@field tileEdge? boolean
---Optional tile size used when tiling the background texture.
...
---Backdrop information used to construct the tooltip backdrop.
---@field backdropInfo AuraContainerTooltipBackdropInfo
---Optional color applied to the backdrop border.
---@field borderColor? colorRGBA
---Optional color applied to the backdrop background.
---@field centerColor? colorRGBA
---Optional offsets applied to the backdrop's TOPLEFT and BOTTOMRIGHT anchor points.
---@field anchorOffsets? AuraContainerTooltipAnchorOffsets

---@class AuraContainerTooltipNineSliceOptions
---Name of the NineSlice layout template to apply, such as 'TooltipDefaultLayout'.
---@field layoutName string
---Optional color applied to the NineSlice border.
---@field borderColor? colorRGBA
---Optional color applied to the NineSlice center region.
---@field centerColor? colorRGBA
---Optional offsets applied to the NineSlice's TOPLEFT and BOTTOMRIGHT anchor points.
---@field anchorOffsets? AuraContainerTooltipAnchorOffsets

---@class AuraContainerTooltipTextureSliceOptions
---Texture asset (file path or atlas name) displayed by the slice.
---@field asset TextureAssetDisk
---Optional texture slice margins. See Texture:SetTextureSliceMargins.
---@field sliceMargins? UITextureSliceMargins
---Optional texture slice mode. See Texture:SetTextureSliceMode.
---@field sliceMode? Enum.UITextureSliceMode
---Optional vertex color applied to the texture.
---@field color? colorRGBA
---Optional offsets applied to the slice's TOPLEFT and BOTTOMRIGHT anchor points.
---@field anchorOffsets? AuraContainerTooltipAnchorOffsets
---Draw layer used when displaying the texture.
---@field drawLayer? string
---Draw layer sublevel used when displaying the texture.
...
---@field quantity? number
---@field houseTextureAtlas? string
...
---Unicode property values for the first code point in a string.
---@class CharacterProperties
---The Unicode code point tested for character properties.
---@field codePoint number
---True if the code point is a letter character with general category L.
---@field isAlphabetic boolean
---True if the code point is a digit character with general category Nd.
---@field isDigit boolean
---True if the code point has the Unicode White_Space property.
---@field isWhitespace boolean
---The short Unicode property value name for the code point's general category.
---@field generalCategory string
---The short ISO 15924 script code for the code point's script.
---@field scriptCode string
---The short Unicode property value name for the code point's block.
---@field blockCode string
...
---@field iconQuestObjective string
...
---A parsed numeric amount and currency from a localized currency string.
---@class CurrencyParseResult
---The numeric amount parsed by the currency parser.
---@field amount number
---The ISO 4217 currency code parsed alongside the amount.
---@field currencyCode string
...
---The maximum number of aura applications represented by the bar.
---@field maxApplications number
---The minimum number of aura applications represented by the bar.
---@field minApplications number
---Optional interpolation method used when updating the bar value.
---@field interpolation? Enum.StatusBarInterpolation

---@class CustomAuraButtonApplicationCountOptions
---Optional formatter used to display aura application counts.
---@field formatter? NumericFormatter

---@class CustomAuraButtonCasterNameOptions
---If true, append realm names to the displayed string.
---@field showRealmName boolean
---If true, apply class coloring to the text.
---@field useClassColors boolean

---@class CustomAuraButtonDispelTypeTextOptions
---Shows the dispel type text for harmful auras.
---@field showWhenHarmful boolean
---Shows the dispel type text for helpful auras.
---@field showWhenHelpful boolean
---Shows the dispel type text for auras that do not have a dispel type.
---@field showWithoutDispelType boolean
---Optional map of dispel type names to custom texts. An empty key can be used for auras without
---a dispel type.
---@field customDispelTextMap? table<string, string>

---@class CustomAuraButtonDispelTypeTextureAsset
---The texture file or atlas to use.
---@field asset TextureAssetDisk
---Resizes the texture to the atlas dimensions. Ignored when asset does not name an atlas.
---@field useAtlasSize boolean
---Optional texture coordinates used when asset does not name an atlas.
---@field texCoords? CustomAuraButtonDispelTypeTextureTexCoords

---@class CustomAuraButtonDispelTypeTextureOptions
---Shows the dispel type texture always, ignoring all other criteria.
---@field showAlways boolean
---Shows the dispel type texture for harmful auras.
---@field showWhenHarmful boolean
---Shows the dispel type texture for helpful auras.
---@field showWhenHelpful boolean
---Shows the dispel type texture for auras that do not have a dispel type.
---@field showWithoutDispelType boolean
---Restricts display of the dispel type texture to either stealable or non-stealable auras. If
---omitted, the dispel type texture may be shown for both.
---@field stealableFilter? Enum.CustomAuraButtonDispelTypeStealableFilter
---The texture style to use.
---@field style Enum.CustomAuraButtonDispelTypeTextureStyle
---Optional map of dispel type names to custom texture assets. The "None" key represents auras
---without a dispel type. Only applies when using the CustomAsset style.
---@field customDispelAssetMap? table<string, CustomAuraButtonDispelTypeTextureAsset>
---Optional map of dispel type names to custom vertex colors. The "None" key represents auras
---without a dispel type.
---@field customDispelColorMap? table<string, colorRGB>
---Optional curve used to determine vertex colors from the aura's dispel type.
...
---The interpolation method used when updating the duration bar.
---@field interpolation? Enum.StatusBarInterpolation
---The direction in which the duration bar progresses.
---@field direction? Enum.StatusBarTimerDirection

---@class CustomAuraButtonDurationTextOptions
---Optional duration text binding to use. The binding is copied before being associated with the
---aura button.
---@field binding? DurationTextBinding
---Optional formatter used to display remaining duration values. Ignored if textFormat is
---specified.
---@field textFormatter? NumericFormatter
---Optional text format configuration applied to the duration text binding. Overrides
---textFormatter when specified.
---@field textFormat? DurationTextBindingFormatOptions
---Optional text color configuration applied to the duration text binding.
...
---@field type Enum.DiscordDisplayNameType
---@field lastOnlineGUID string
...
---@class DurationTextBindingColorOptions
---@field curve LuaColorCurveObject
---@field property Enum.DurationTextBindingProperty

---@class DurationTextBindingFormatComponent
---The duration property sampled for this format component.
---@field property Enum.DurationTextBindingProperty
---The numeric formatter used to convert the sampled duration property into text.
---@field formatter NumericFormatter

---@class DurationTextBindingFormatOptions
---@field formatString string
---@field components DurationTextBindingFormatComponent[]
...
---@field cost number
---Will be nil if there is no target house or information for it is unavailable
---@field max? number
---Will be nil if there is no target house or information for it is unavailable
---@field current? number

---@class HousingBlueprintBudgetInfo
---Map of budget type to budget entry
---@field exteriorBudgets table<Enum.HousingBudgetType, HousingBlueprintBudgetEntry>
---Map of budget type to budget entry
---@field interiorBudgets table<Enum.HousingBudgetType, HousingBlueprintBudgetEntry>
...
---@field recordID number
---@field name string
---Total instances of the object contained within or required by the Blueprint
---@field total number
---The number of instances of this item that the player doesn't own, or have unlocked, or have
---available for use (depending on the type of content)
---@field numMissing number
---True for content that isn't valid for use whether it's owned or not, for example exterior
---types of the opposite faction than the house's neighborhood
...
---@field blueprintID integer
---@field shareCode string
---@field name string
---@field blueprintType Enum.HousingBlueprintType
---@field creationTime integer
---True if this blueprint is an automatically saved backup, rather than a manually player-saved
---blueprint
...
---Base information about an object in the Catalog; For info for a specific owned stack of this
---object, see HousingCatalogEntryVariantInfo
...
---The total number of instances of this entry that exist in storage across all variants; Does
---not include unredeemed instances (see remainingRedeemable)
...
---houses and plots, across all variants
...
---@field recordID number
---@field entryType Enum.HousingCatalogEntryType
---Hashed value used to identify and differentiate stacks that are the same type and record, but
---have some variant-specific difference (e.g. dye colors)
---@field variantIdentifier number

---Represents a single stack of instances of an object in the Catalog, that are all of a
---specific variation; For example, a stack of undyed chairs, or blue-dyed tables
---@class HousingCatalogEntryVariantInfo
---@field entryVariantID HousingCatalogEntryVariantID
---The number of instances of this specific variant that exist in storage
---@field numStored number
---Dye slot information for this variant; Empty for entries that can't be dyed
...
---If true, search will only return categories/subcategories that the player has something
---stored under
...
---The name of the dye color (if any) that is currently applied to this slot
...
---Minimum input value that this rule applies to.
---@field threshold number
---If specified, the input value is rounded to multiples of this step before processing.
---@field step? number
---If specified, the rounding mode to apply to the step.
---@field rounding Enum.NumericRuleFormatRounding
---If specified, clamps the input value to this value if lesser after rounding.
---@field min? number
---If specified, clamps the input value to this value if greater after rounding.
---@field max? number
---The format string to apply at this threshold. If no components are specified, this can
---include at-most one numeric format specifier - else, should contain an equal number of format
---specifiers to elements in the components array.
---@field format string
---Vector of component descriptions for this rule.
---@field components? NumericRuleFormatComponent[]

---@class NumericRuleFormatComponent
---If specified, divide the value by this amount and use the resulting quotient. This is applied
---before modulo division.
---@field div? number
---If specified, divide the value by this amount and use the resulting remainder. This is
---applied after quotient division.
---@field mod? number
---If specified, round the value to multiples of this step. This is applied after all division.
---@field step? number
---If specified, rounding mode to apply to the step.
---@field rounding Enum.NumericRuleFormatRounding
...
---Maximum table nesting depth allowed during recursive copying.
---@field maxTraversalDepth number
---Wrap function values in secure closures that call through using the original function taint.
---@field wrapUntrustedFunctions boolean
---If true, preserve references to script objects in the copied output. By default, script
---objects will raise an error.
---@field permitScriptObjects boolean

---@class SecureDelegateOptions
---Wrap function arguments in secure closures that call through using the original function
---taint.
---@field wrapUntrustedFunctions boolean
...
---False if recharging is not active (ex: at maximum available charges, start time or duration
---are zero); True otherwise
---@field isActive boolean
...
---False if cooldown is not active (ex: not enabled, or startTime or duration are 0); True
---otherwise
---@field isActive boolean
---Rate at which cooldown UI should update
...
---If cooldown is active, time started; 0 if no cooldown
---@field startTime number
---Cooldown duration in seconds if active; 0 if cooldown is inactive
---@field duration number
---Rate at which cooldown UI should update
---@field modRate number
---False if cooldown is not active (startTime or duration are 0); True otherwise
---@field isActive boolean
---True if this cooldown should be displayed in-place of the normal action cooldown (ex: the LoC
---cooldown expires later than the normal cooldown); False otherwise
...
---Structure associating a key with a scheduled time.
---@class TimedSignalMapEntry
---The key that has been scheduled.
---@field key number
---Absolute time, on the same clock as GetTime(), at which the key is scheduled to signal.
---@field time number
...
---@alias PingPinFrameAddedCallback fun(region: ScriptRegion, uiTextureKit: string, isWorldPoint: boolean, actionInfo?: PingActionUIInfo)
...
---@alias SendMacroPingCallback fun(macroInfo: PingMacroInfo)

---@alias TickerCallback fun(cb: TimerCallback)

---Invoked by a timed signal map when a scheduled time has arrived.
---@alias TimedSignalMapCallback fun(key: number)
