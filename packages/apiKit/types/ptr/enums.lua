...
---@enum Enum.BreakType
Enum.BreakType = {
  ---Locate extended grapheme cluster boundaries, which are treated as user-perceived character
  ---units.
  Character = 0,
  ---Locate word boundaries for selection, search, and whole-word operations, including locale
  ---dictionary support where available.
  Word = 1,
  ---Locate sentence boundaries while handling cases such as periods in numbers or abbreviations
...
Enum.CollationStrength = {
  ...
  Secondary = 1,
  ---Compare case, letter variants, and similar tertiary differences after primary and secondary
  ---differences match.
  Tertiary = 2,
...
Enum.CustomAuraButtonUpdateMode = {
  ---Applies a newly assigned aura, resetting presentation state rather than transitioning from
  ---the previous aura.
  Assignment = 0,
...
Enum.EncounterTimelineEventSource = {
  ...
  Encounter = 0,
  ---Source used for events added by Lua scripting APIs. This is used to apply API restrictions;
  ---the Pause/Cancel/ResumeScriptEvent functions only work on Script events.
  Script = 1,
...
Enum.EncounterTimelineViewType = {
  ---Configures the view for an empty timeline display. This mode is recommended for user addons
  ---that take full control of the timeline display.
  None = 0,
...
Enum.FontStringScaleAnimationMode = {
  ...
  ---non-slug fonts may cause significant pixelation of the font text, Layout of the text is not
  ---adjusted as the animation progresses, and the direction of scaling respects horizontal and
  ---vertical justification settings (eg. text at the bottom-right scaling up grows toward the
  ---top-left).
  Vertex = 1,
}
...
Enum.HousingFixtureDecorAction = {
  ...
  Store = 0,
  ---Detach any currently-attached decor, and leave it floating in its current position; It will
  ---NOT be re-attached to the new Exterior Fixtures
  Detach = 1,
}
...
Enum.NumberStyle = {
  ...
  Percent = 2,
  ---Use generic currency format with a currency symbol and non-accounting minus-sign negatives.
  Currency = 3,
}
...
Enum.SecondsFormatterIntervalWhitespace = {
  ...
  Preserve = 0,
  ---Strip whitespace from interval unit strings. Some locales override this setting (eg. deDE,
  ---ruRU).
  Strip = 1,
...
Enum.StatusBarRenderMode = {
  ...
  Linear = 0,
  ---Render the status bar by driving the managed texture's radial progress fill percent instead
  ---of resizing the texture anchors.
  Radial = 1,
}
...
Enum.UnitAuraSortRule = {
  ...
  Default = 1,
  ---Sorts auras according first by whether or not the aura was applied by another player, else
  ---by expiration time (longest to shortest), and finally by aura instance ID.
  BigDefensive = 2,
  ---Sorts auras according first by whether or not the aura was applied by the player, else
  ---whether or not the player can apply the aura, then by expiration time (soonest to longest,
  ---followed by permanent auras), and finally by aura instance ID.
  Expiration = 3,
...
Enum.UnitDamageAbsorbClampMode = {
  ...
  MissingHealth = 0,
  ---Clamp damage absorb values to the amount of missing health. Incoming heals do not decrease
  ---missing health.
