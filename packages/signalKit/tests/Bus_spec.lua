...
describe("SignalKit named buses", function()
  ...
  it("names a bus method called without its receiver", function()
    ...
    expectRefusalAtCaller("SignalKit.Bus:Subscribe must be called on a SignalKit bus", function()
...
describe("SignalKit bus topic policy", function()
  ...
  it("never prints a secret refusal reason", function()
    ...
    assert.is_false(ok)
    assert.is_not_nil(string.find(tostring(message), "the validator gave a secret reason", 1, true))
  ...
  it("refuses a secret bus name or topic before comparing it", function()
    ...
    expectRefusalAtCaller("SignalKit.Bus:DeclareTopic topic must not be a secret value", function()
