...
describe("SignalKit errors", function()
  ...
  it("names the misuse when a signal method is called without a receiver", function()
    ...
    expectErrorContaining("SignalKit:DisconnectAll must be called on a signal instance", function()
      SignalKit.DisconnectAll()
    end)
  end)

  it("names the misuse when a connection method is called without a receiver", function()
    expectErrorContaining("SignalKit:Disconnect must be called on a connection handle", function()
      SignalKit.Connection.Disconnect()
    end)

    expectErrorContaining("SignalKit:IsConnected must be called on a connection handle", function()
  ...
  it("points receiver and argument errors at the calling line", function()
    ...
    for index = 1, #calls do
      ...
      assert.is_false(ok)
      assert.is_not_nil(string.find(message, "packages/signalKit/tests/Errors_spec.lua:", 1, true))
