...
local function proxyTree(tree, overrides)
  ...
  for _, name in ipairs({
    ...
    proxy[name] = overrides[name] or function(_, ...)
...
describe("WidgetKit and secret booleans", function()
  ...
  it("refuses a secret anchor offset instead of defaulting it", function()
    ...
    assertReportedAtCaller("WidgetKit.Anchor.Normalize x must not be a secret value", function(mark)
      mark()
      WidgetKit.Anchor.Normalize(frame, "TOP", parent, "TOP", secret, 0)
    end)
    assertReportedAtCaller("WidgetKit.Anchor.Normalize y must not be a secret value", function(mark)
      mark()
