    ...
    args = {
      ...
      frame = {
        ...
        args = {
          ...
          color = option({ type = "color", name = "Border", order = 4, hasAlpha = true }, "color"),
        },
      },
      channels = option({
        type = "multiselect",
      ...
      toggleKey = option({ type = "keybinding", name = "Toggle key", order = 5 }, "toggleKey"),
      reset = {
        type = "execute",
        name = "Reset",
        order = 6,
...
describe("WidgetKit options renderer", function()
  ...
  it("shows the current values and the hints", function()
    ...
    assert.are.equal("GameFontHighlightLarge", rendering:GetWidget("intro").text:GetFontObject())
  end)
...
describe("WidgetKit options renderer without OptionsKit", function()
  ...
  it("raises at the caller", function()
    ...
    TestEnv.expectErrorContaining("WidgetKit:RenderOptions requires OptionsKit API 1", function()
      WidgetKit:RenderOptions({}, window)
    end)
  end)
end)
