...
describe("HookKit error levels", function()
  ...
  it("points package argument errors at the caller", function()
    assertReportedAtCaller("HookKit:ForAddon addonName must be a non-empty string", function(mark)
      mark()
      HookKit:ForAddon("")
    end)
  ...
  it("points lookup argument errors at the caller", function()
    ...
    for _, method in ipairs({ "Unhook", "IsHooked", "Original" }) do
      ...
      assertReportedAtCaller(prefix .. " globalName must be a non-empty string", function(mark)
        mark()
        scope[method](scope, "")
      end)
    end
