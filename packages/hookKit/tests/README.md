...
- limits: the default of 256 enforced with `"full"`, `options.maxHooks` smaller and larger, `HookKit.UNBOUNDED` holding 600 hooks, the first `ForAddon` fixing the limit (a later call that agrees or names none returns the scope, one naming a different limit is refused at the caller and changes nothing), invalid, unknown and secret values refused at the caller with nothing changed (also for an existing addon scope), and the sentinel and every scope's limit kept across an upgrade;
