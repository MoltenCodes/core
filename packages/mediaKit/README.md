...
- **`List`** returns a cached sorted array, shared by every caller and read-only, rebuilt only after a registration of its type (for fonts, also when the client's script changed).
