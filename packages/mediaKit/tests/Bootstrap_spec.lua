...
describe("MediaKit bootstrap", function()
  ...
  it("upgrades in place and keeps the set limits and the UNBOUNDED sentinel", function()
    ...
    assert.are.same({ maxEntriesPerType = 4096, maxConsumers = unbounded }, upgraded:GetLimits())
  ...
  it("upgrades revision 1 in place and keeps its entries, defaults and limits", function()
    ...
    assert.is_false(ok)
    assert.is_truthy(
      tostring(message):find("MediaKit:SetLimits limits.<table> is not a recognised limit", 1, true)
    )
  end)

  it("upgrades revision 2 in place and keeps its entries, defaults and limits", function()
    TestEnv.Reset()
    TestEnv.InstallWowApi()
    TestEnv.SetClientLocale(TestEnv.DEFAULT_CLIENT_LOCALE)
    require("Registry")
    ...
    -- Absence of an optional argument is now tested with `type`.
    defaults:Set("statusbar", nil)
  ...
  it("upgrades revision 3 in place and refuses a secret anyScript at once", function()
    ...
    TestEnv.expectErrorContaining("MediaKit:Fetch anyScript must not be a secret value", function()
