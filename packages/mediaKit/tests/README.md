...
- duplicate embedded loading, Registry publication, yielding to a newer revision, missing Registry, missing SignalKit, an incomplete facade, and an in-place upgrade that keeps entries, lists, connections, defaults, adoption, the single subscription and mirroring, an upgrade that keeps the set limits and the `UNBOUNDED` sentinel, an upgrade of a revision 1 copy by the current file, and a refusal of state holding an invalid limit;
