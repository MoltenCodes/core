...
describe("LifecycleKit package bootstrap", function()
    ...
    it("registers LifecycleKit API 1 revision 8", function()
        ...
        assert.are.equal(8, LifecycleKit.REVISION)
    end)
    ...
    it("does not downgrade a newer compatible embedded revision", function()
        ...
        local future = Registry:Register("lifecycleKit", 1, 9)
        future.API = 1
        future.REVISION = 9
    ...
    it("refuses a newer revision that lacks the combat gate surface", function()
        ...
        -- A revision 9 that publishes only the revision 6 surface is not a
