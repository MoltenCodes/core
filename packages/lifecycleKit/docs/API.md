...
Revisions 8 to 10 keep schema 3. Upgrading from revision 7, 8 or 9 replaces its shared host watchers, which would otherwise keep calling the older logout handler: revision 7's closes no HookKit scope and no bus, revision 8's no CommandKit scope and revision 9's no CommKit scope.
