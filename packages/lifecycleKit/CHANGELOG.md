...
- Added `instance:OnCombatStart(callback)` and `instance:OnCombatEnd(callback)`, repeating subscriptions delivered after the shared state flips, only to addons that have reached `loaded` and are not halted or shut down. A combat transition allocates nothing once the subscriptions exist.
...
- Package state schema 3. An in-place upgrade from an older revision adds the combat flag, the creation-ordered instance list and the new per-instance fields, and replaces the older revision's shared host watchers, which would otherwise keep calling handlers that know nothing of halting or of the combat queue.
