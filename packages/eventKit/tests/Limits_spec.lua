...
describe("EventKit limits", function()
    ...
    it("enforces the default of 64 unit-filter Frames", function()
        ...
        assert.is_false(ok)
        assert.is_not_nil(
            string.find(tostring(message), "refusing to create more than 64", 1, true)
        )
    ...
    it("honours a raised maxUnitFrames", function()
        ...
        assert.is_false(ok)
        assert.is_not_nil(
            string.find(tostring(message), "refusing to create more than 100", 1, true)
        )
    end)
    ...
    it("refuses unknown limits and non-table arguments at the caller", function()
        expectCallerError(
            "EventKit:SetLimits limits.maxFrames is not a recognised limit",
            function()
