...
describe("EventKit errors", function()
  ...
  it("points argument errors at the calling line, not at EventKit", function()
    ...
    for index = 1, #calls do
      ...
      assert.is_false(ok)
      assert.is_not_nil(string.find(message, "packages/eventKit/tests/Errors_spec.lua:", 1, true))
  ...
  it("names the misuse when a connection method is called without a receiver", function()
    ...
    expectErrorContaining("EventKit:Disconnect must be called on a connection handle", function()
      EventKit.Connection.Disconnect(nil)
    end)

    expectErrorContaining("EventKit:IsConnected must be called on a connection handle", function()
  ...
  it("names the package instead of a misleading line for host failures", function()
    ...
    assert.is_false(ok)
    assert.are.equal("EventKit: requires the World of Warcraft CreateFrame API", tostring(message))
  end)
