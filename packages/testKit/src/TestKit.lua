...
local function getTimerScope(methodName, level)
  ...
  if scope == false or scope:IsClosed() then
    local TimerKit = findPackage("timerKit", "TimerKit", FOUND_TIMERKIT_API, methodName, level + 1)
...
local function getEventScope(methodName, level)
  ...
  if scope == false or scope:IsClosed() then
    local EventKit = findPackage("eventKit", "EventKit", FOUND_EVENTKIT_API, methodName, level + 1)
...
local function compareValues(actual, expected, depth, trail)
  ...
  if type(actual) ~= "table" or type(expected) ~= "table" then
    return false, describeValue(actual) .. " where " .. describeValue(expected) .. " was expected"
  end
...
local function abandonStep(record)
  ...
  if record.stepIndex < record.afterStart then
    recordFailure(record, STATUS_TIMEOUT, "the test did not finish within " .. limit .. " seconds")
  else
    recordFailure(
      record,
      STATUS_FAILED,
...
local function validateLimitUpdate(limits, level)
  ...
  while type(key) ~= "nil" do
    ...
    elseif not isLimitValue(key, value, UNBOUNDED) then
      error(
        "TestKit:SetLimits limits." .. key .. " must be a positive integer or TestKit.UNBOUNDED",
        level
      )
    end
    key = next(limits, key)
