...
--
--   (a) a LifecycleKit whose `CLOSES_ADDON_SCOPES` names "commandKit" closes it
--       after the addon's shutdown callbacks;
--   (b) an older LifecycleKit: CommandKit's own `OnShutdown` subscription does;
--   (c) no LifecycleKit, but EventKit: CommandKit's `PLAYER_LOGOUT` watcher does;
...
-- (b) CommandKit's earlier subscription has already closed it.

---Register a fresh command in `scope` and return its name.
---@param scope table
---@param name string?
---@return string name
local function registerCommand(scope, name)
  name = name or "refresh"
  assert.is_true((scope:Register(name, { handler = function() end })))
  return name
end

---Subscribe to the addon's shutdown after CommandKit did, and report whether the
...
describe("CommandKit logout close", function()
  after_each(TestEnv.Reset)

  describe("(a) LifecycleKit names commandKit in CLOSES_ADDON_SCOPES", function()
    it("leaves the scope to LifecycleKit, open during the shutdown callbacks", function()
      ...
      local target = registerCommand(scope)
      ...
      assert.is_false(scope:IsRegistered(target))
  ...
  describe("(b) an older LifecycleKit", function()
    ...
    it("treats a list that does not name commandKit as an older LifecycleKit", function()
      local CommandKit = TestEnv.NewPackage()
  ...
  describe("(c) EventKit without LifecycleKit", function()
    it("closes every addon scope on PLAYER_LOGOUT through one watcher", function()
      ...
      local first = CommandKit:ForAddon("MyAddon")
      local second = CommandKit:ForAddon("OtherAddon")
      local manual = CommandKit:CreateScope()
      local target = registerCommand(first)
      registerCommand(manual, "manual")

      local watch = rawget(rawget(CommandKit, "_state"), "logoutWatch")
      ...
      assert.is_false(first:IsRegistered(target))
  ...
  describe("(d) neither LifecycleKit nor EventKit", function()
    it("arranges nothing, and the consumer's own CloseAddonScopes closes the scope", function()
      ...
      local scope = CommandKit:ForAddon("MyAddon")
      registerCommand(scope)

      assert.is_nil(Registry:Find("eventKit", 1))
      assert.are.equal("none", rawget(scope, "_logoutCloser"))
      assert.is_false(rawget(rawget(rawget(CommandKit, "_state"), "logoutWatch"), "connection"))
      TestEnv.Logout()
  ...
  describe("re-evaluation", function()
    ...
    it("moves a scope from the PLAYER_LOGOUT watcher to a LifecycleKit that loads later", function()
      local CommandKit = TestEnv.NewPackage()
      TestEnv.LoadEventKit()
      local scope = CommandKit:ForAddon("MyAddon")
  ...
  describe("upgrades", function()
    it("arranges the logout close of addon scopes a revision 1 layout carried", function()
      ...
      require("SchemaKit")
      require("OptionsKit")
      local CommandKit = TestEnv.LoadRevision(1)
      local scope = CommandKit:ForAddon("MyAddon")
      local target = registerCommand(scope)
      ...
      rawset(rawget(CommandKit, "_state"), "logoutWatch", nil)
    ...
    it("keeps the PLAYER_LOGOUT watcher across an upgrade without connecting another", function()
      ...
      TestEnv.LoadRevision(CommandKit.REVISION + 1)
      CommandKit:ForAddon("MyAddon")
      CommandKit:ForAddon("OtherAddon")

      assert.are.equal(connection, rawget(watch, "connection"))
      assert.are.equal(1, rawget(watch, "scope"):GetActiveCount())
      TestEnv.Logout()
      assert.is_true(scope:IsClosed())
      assert.is_true(CommandKit:ForAddon("OtherAddon"):IsClosed())
      assert.are.same({}, TestEnv.TakeReportedErrors())
    end)
