...
describe("CommandKit BindOptions", function()
  ...
  it("sets values parsed per kind", function()
    ...
    assert.are.same({ "frame.anchor = CENTER (Centre)" }, run("/myopts set frame.anchor CENTER"))
    assert.are.same({ "frame.color = 0.00 0.50 1.00 1.00" }, run("/myopts set frame.color 0 0.5 1"))
    assert.are.same(
  ...
  it("prints the schema refusal of an out-of-range value", function()
    ...
    assert.are.same({ "/myopts set: expected on, off or toggle" }, run("/myopts set enabled maybe"))
    assert.are.same(
  ...
  it("refuses unknown, hidden and disabled options with a message", function()
    ...
    assert.are.same({ '/myopts get: unknown option "hiddenFlag"' }, run("/myopts get hiddenFlag"))
