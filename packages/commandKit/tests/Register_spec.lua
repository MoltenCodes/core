...
describe("CommandKit registration", function()
  ...
  it("writes SlashCmdList[<key>] and SLASH_<key>1 with a key from the addon and command", function()
  ...
  it("unregisters, leaves the globals inert, and reuses the key on a later registration", function()
    local calls = 0
  ...
  it("refuses malformed names and specs at the caller", function()
    ...
    TestEnv.expectErrorContaining('spec.subcommands declares "dup" twice', function()
      scope:Register("x", { subcommands = { dup = { handler = noop }, DUP = { handler = noop } } })
    end)
  ...
  it("refuses sub-commands nested deeper than MAX_DEPTH", function()
    ...
    local three = { subcommands = { c = { subcommands = { b = { subcommands = { a = leaf } } } } } }
