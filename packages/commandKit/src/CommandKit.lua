...
local function slashDispatch(key, text, _)
  ...
  if frame == nil then
    writeLine(rawget(record, "_scope"), rawget(record, "_path") .. ": commands nested too deeply")
    return
  end
...
local function newOptionHandlers(tree)
  ...
  function handlers.exec(context, path, confirmation)
    ...
    if asks and confirmation ~= "confirm" then
      ...
      contextPrint(
        context,
        "Type " .. contextGetCommandPath(context) .. " " .. node.path .. " confirm to run it."
      )
      return
    end
...
local function refuseConflictingOptions(scope, options, limits, label, level)
  ...
  for index = 1, #SCOPE_OPTION_NAMES do
    local field = SCOPE_OPTION_NAMES[index]
    if type(rawget(options, field)) ~= "nil" and limits[field] ~= rawget(scope, "_" .. field) then
      error(
        label .. "." .. field .. " differs from the limit this addon's scope was created with",
        level
      )
    end
  end
...
local function validateLimitUpdate(limits, level)
  ...
  for key, value in next, limits do
    ...
    elseif not secret and value == UNBOUNDED then
      error(label .. " cannot be CommandKit.UNBOUNDED: " .. LIMIT_UNBOUNDED_REFUSALS[key], level)
