...
- **Snapshots** hand your reader a `fill(key, value)` function and diff what it reports against the previous refresh. The three result arrays are reused, and a refresh in which nothing changed allocates nothing. A failed read rolls back the keys it added and restores the values it changed, so a snapshot never exceeds its `maxEntries` and the next successful refresh still reports every change.
- **Secret values** (Retail 12.x) cannot be compared with a value of their own type or used as table keys; they can be stored as values. Check `ClientKit:IsSecret` or `issecretvalue` before handing client values to a cache; a snapshot's `fill` refuses secrets itself.
