...
- 88 specs, including allocation guards, a 5,000-step property test against a naive LRU model that also checks the free-list bound, an in-place upgrade spec, and regression specs for repeated failing reads, a change made during a failed refresh, a refused `ClearOn` registration and secret values.
