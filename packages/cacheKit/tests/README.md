...
- secret values outside snapshots: a secret `maxEntries`, queue `capacity` and `SetLimits` value refused at the caller before any comparison, and a secret value stored, read back and memoised without being compared;
...
| `SecretValues_spec.lua` | secret keys and values refused by `fill`; secret limits refused; secret values stored and memoised |
