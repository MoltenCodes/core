...
- secret values outside snapshots: a secret `maxEntries`, queue `capacity`, `ttlSeconds` (`NewTtl`, `Memoize`, `PutNegative`), queue `overflow` and `SetLimits` value refused at the caller before any comparison or key use, and a secret value stored, read back and memoised without being compared;
...
| `SecretValues_spec.lua` | secret keys and values refused by `fill`; secret limits, lifetimes and overflow policies refused; secret values stored and memoised |
