...
describe("CacheKit lazy tree", function()
  ...
  it("reads as empty once closed and refuses Get", function()
    ...
    assert.are.equal(0, tree:Clear())
    assert.are.equal(0, tree:GetCount())
    TestEnv.expectErrorContaining("CacheKit.LazyTree:Get cannot expand a closed tree", function()
      tree:Get("a")
    end)
  end)
  ...
  it("validates paths on every path method", function()
    local tree = CacheKit:Lazy(newCountingResolver())
    TestEnv.expectErrorContaining("CacheKit.LazyTree:Get needs at least one path part", function()
      tree:Get()
    end)
    TestEnv.expectErrorContaining(
    ...
    TestEnv.expectErrorContaining("CacheKit.LazyTree:Get path part 1 must not be NaN", function()
      tree:Get(0 / 0)
    end)
    TestEnv.expectErrorContaining(
  ...
  local function runRandomizedWorkload(maxEntries, seed)
    ...
    local function checkNode(node, parent)
      ...
      elseif node ~= tree._root then
        assert.is_true(hasExpandedDescendant, "structural node without an expanded descendant")
      end
