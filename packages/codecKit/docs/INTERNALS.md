...
A newer revision keeps `_state` (limits, the `UNBOUNDED` sentinel and pool) and rewrites the facade's functions and constants. Work records and sinks exist only during a call, so no object outlives the revision that built it. No revision so far has changed the state layout: revision 2 inherits the state revision 1 built unchanged, and so will a later revision that keeps schema 1.
