...
describe("LogKit and secret values", function()
  ...
  it("refuses a secret message at the caller", function()
    local secret = Env.NewSecretValue()
    Env.expectErrorContaining("LogKit.Logger:Warn message must not be a secret value", function()
      logger:Warn(secret)
    end)
  ...
  it("refuses a secret addon name, level, sink and chat frame at the caller", function()
    ...
    Env.expectErrorContaining("LogKit.Logger:SetLevel level must not be a secret value", function()
      logger:SetLevel(secret)
    end)
    Env.expectErrorContaining("LogKit:SetGlobalLevel level must not be a secret value", function()
