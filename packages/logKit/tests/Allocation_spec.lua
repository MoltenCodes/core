...
describe("LogKit allocation #allocation", function()
  ...
  it("allocates nothing for an enabled bare message delivered to a sink and the journal", function()
    local count = 0
  ...
  it("allocates nothing to check IsEnabled and GetLevel", function()
    ...
    assert.is_true(allocated < THRESHOLD_KILOBYTES, "IsEnabled allocated " .. allocated .. " KiB")
  end)
