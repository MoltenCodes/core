...
describe("CommKit error levels", function()
  ...
  it("points facade receiver and argument errors at the caller", function()
    ...
    assertReportedAtCaller("CommKit:ForAddon addonName must be a non-empty string", function(mark)
      mark()
      CommKit:ForAddon("")
    end)
    assertReportedAtCaller(
  ...
  it("points SyncSet method errors at the caller", function()
    ...
    assertReportedAtCaller("CommKit.SyncSet:OnChanged callback must be a function", function(mark)
      mark()
      sync:OnChanged(nil)
    end)
  end)
