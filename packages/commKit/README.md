...
- **Bounded against strangers.** Reassembly is bounded in streams (64), bytes per sender (16 KB), streams per sender (4) and time (30 s since the last chunk); a sender who leaves the group loses their streams. A hostile header is refused before anything is stored, and a dropped stream is reported once.
