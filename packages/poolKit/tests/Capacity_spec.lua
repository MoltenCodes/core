...
describe("PoolKit capacity", function()
    ...
    it("supports an explicit unbounded retention escape hatch", function()
        ...
        local values = {}
        for i = 1, 300 do values[i] = pool:Acquire() end
        for i = 1, 300 do pool:Release(values[i]) end
    ...
    it("prewarms to a target and is idempotent", function()
        local PoolKit = Env.NewPackage()
        local pool = PoolKit:New({ create = function() return {} end, maxRetained = 8 })
        ...
        assert.has_error(function() pool:Prewarm(9) end)
    end)

    it("trims immediately when maxRetained is lowered", function()
        local PoolKit = Env.NewPackage()
        local pool = PoolKit:New({ create = function() return {} end, prewarm = 5, maxRetained = 5 })
