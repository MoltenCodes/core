...
describe("Registry bootstrap", function()
  ...
  it("rejects a public alias that is not a Registry facade at all", function()
    -- Registry's bootstrap handshake happens through the global table, so this spec sets it up there directly.
    -- selene: allow(global_usage)
    rawset(_G, TestEnv.NAMESPACE_KEY, {
      Registry = "not a facade",
    })

    local ok, message = pcall(require, "Registry")

    assert.is_false(ok)
    assert.are.equal("Registry: MoltenCodes.Registry is owned by an incompatible value", message)
  end)
  ...
  it("rejects bootstrap fields supplied only through a metatable", function()
    ...
    assert.is_false(ok)
    assert.is_not_nil(string.find(tostring(message), "bootstrap state is incompatible", 1, true))
  end)
