...
describe("Registry retirement and migration", function()
    ...
    it(
        "asks the outgoing copy to retire exactly once, before the incoming one registers",
        function()
            local Registry = TestEnv.NewRegistry()
            local calls = {}
    ...
    it("reports a failing retire hook and continues from no hand-over", function()
        ...
        assert.is_not_nil(
            string.find(reported[1], "MoltenCodes DemoKit retire hook failed", 1, true)
        )
    ...
    it(
        "resolves the outgoing copy's entry points and instances to the incoming revision",
        function()
            local Registry = TestEnv.NewRegistry()
