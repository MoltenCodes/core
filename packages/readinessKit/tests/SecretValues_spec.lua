...
describe("ReadinessKit and secret values", function()
  ...
  it("refuses a secret gate name at the caller's line before comparing it", function()
    ...
    assertReportedAt(getLine, "ReadinessKit:Get name must not be a secret value", getOk, getValue)
  end)
  ...
  it("returns false from Probe for a secret answer and caches it like a negative answer", function()
    local calls = 0
    local secret = TestEnv.NewSecretValue()
    local gate = ReadinessKit:Gate("spells", function()
      calls = calls + 1
