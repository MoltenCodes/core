...
- secret values through a local `issecretvalue` stub installed after load: every kind refuses a secret with rule `secret` before touching it (the stand-in secret raises on any use), nested secrets, `Assert` messages and `Apply`, and a secret `SetLimits` value refused at the caller's line before it is compared;
