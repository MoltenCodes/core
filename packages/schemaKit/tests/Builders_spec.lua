...
describe("SchemaKit builders", function()
  ...
  describe("string", function()
    ...
    it("refuses invalid specs", function()
      ...
      TestEnv.expectErrorContaining("SchemaKit.string min must not be greater than max", function()
      ...
      TestEnv.expectErrorContaining("SchemaKit.string oneOf must list strings only", function()
        S.string({ oneOf = { "a", 1 } })
      end)
      TestEnv.expectErrorContaining("SchemaKit.string oneOf values must not repeat", function()
        S.string({ oneOf = { "a", "a" } })
      end)
      TestEnv.expectErrorContaining("SchemaKit.string is called with a dot, not a colon", function()
        S:string({})
      end)
    end)
  end)
  ...
  describe("number", function()
    ...
    it("refuses invalid specs", function()
      ...
      TestEnv.expectErrorContaining("SchemaKit.number min must not be greater than max", function()
  ...
  describe("array", function()
    ...
    it("refuses invalid specs", function()
      ...
      TestEnv.expectErrorContaining("SchemaKit.array max must be a non-negative integer", function()
        S.array({ of = S.any(), max = -1 })
      end)
      TestEnv.expectErrorContaining("SchemaKit.array min must not be greater than max", function()
  ...
  describe("map", function()
    it("checks every key and value", function()
      ...
      assert.are.same({ false, "max" }, { check(node, { a = 1, b = 2, c = 3, d = 4, e = 5 }) })
    end)

    it("requires keys, values and max", function()
      TestEnv.expectErrorContaining("SchemaKit.map keys must be a SchemaKit schema node", function()
  ...
  describe("optional", function()
    ...
    it("refuses a cyclic default and nested optionals", function()
      ...
      TestEnv.expectErrorContaining("SchemaKit.optional schema is already optional", function()
  ...
  describe("oneOf", function()
    ...
    it("refuses invalid alternative lists", function()
      TestEnv.expectErrorContaining("SchemaKit.oneOf alternatives must not be empty", function()
        S.oneOf({})
      end)
      TestEnv.expectErrorContaining(
