...
describe("SchemaKit limits", function()
  ...
  it("honours maxPatternCaptures, which can only be lowered", function()
    ...
    TestEnv.expectErrorContaining("SchemaKit.string pattern is not a valid Lua pattern", function()
  ...
  it("refuses UNBOUNDED where a ceiling protects the stack, the matcher or a message", function()
    ...
    for _, case in ipairs(cases) do
      ...
      assertReportedAt(
        line,
        "SchemaKit:SetLimits limits." .. case[1] .. " cannot be SchemaKit.UNBOUNDED: " .. case[2],
        ok,
        value
      )
    end
  ...
  it("refuses values past a ceiling and invalid values at the caller, changing nothing", function()
    local line
    local ok, value = pcall(function()
      line = currentLine() + 1
    ...
    assertReportedAt(tableLine, "SchemaKit:SetLimits limits must be a table", tableOk, tableValue)
